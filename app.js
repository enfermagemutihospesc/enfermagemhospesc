...
function _gerarPDFRelatorio(titulo, dados, narrativa, periodoRotulo){
  try {
    ...
    // A causa raiz do texto estourando a margem foi eliminada com a fonte
    // real embutida (_carregarFonteRelatorio) — cálculo de largura e
    // renderização agora usam exatamente a mesma fonte em qualquer
    // visualizador. Mantém uma margem residual de 2% só por prudência
    // (ex.: arredondamentos internos do jsPDF), não é mais a defesa principal.
    const _MARGEM_SEGURANCA_FONTE = 0.98;
    ...
    const secTitulo = (nome, cor=[13,71,161]) => {
      ...
      doc.setFont('LiberationSans','bold'); doc.setFontSize(10); doc.setTextColor(255,255,255);
    ...
    const linha = (label, valor, unidade='', destaque=false) => {
      _reserva(7);
      doc.setFont('LiberationSans', destaque ? 'bold' : 'normal'); doc.setFontSize(8.5);
    ...
    const tabela = (cabecalho, linhas, larguras) => {
      ...
      if(alturaTotal > espacoDisponivel && espacoDisponivel < 50){
        ...
        if(_secaoAtual){
          doc.setFont('LiberationSans','italic'); doc.setFontSize(8); doc.setTextColor(120,120,120);
      ...
      const desenharCabecalho = () => {
        doc.setFillColor(13,71,161); doc.rect(M, y, L, HCAB, 'F');
        doc.setFont('LiberationSans','bold'); doc.setFontSize(8); doc.setTextColor(255,255,255);
        let cx = M;
      ...
      linhas.forEach((row, ri) => {
        ...
        doc.setFont('LiberationSans','normal'); doc.setFontSize(8); doc.setTextColor(40,40,40);
        let cx = M;
        row.forEach((cell, i) => {
    ...
    const paragrafo = (texto, fontSize=9) => {
      ...
      doc.setFont('LiberationSans','normal'); doc.setFontSize(fontSize); doc.setTextColor(40,40,40);
      linhas.forEach(l => {
        _reserva(lhMm + 2);
        doc.text(l, M, y);
        y += lhMm;
    ...
    const narrativaFormatada = (texto, fontSize=9) => {
      ...
      linhasBrutas.forEach(linhaBruta => {
        ...
        if(mHead){
          doc.setFont('LiberationSans','bold'); doc.setFontSize(fontSize+1); doc.setTextColor(26,107,58);
    ...
    doc.setTextColor(255,255,255); doc.setFont('LiberationSans','bold'); doc.setFontSize(15);
...
async function _pitGerarTexto(form){
  ...
  if(!ocupadosNums.length){
    ...
    ocupadosNums.forEach(n => {
      ...
      // cadastro do leito — mesmo critério usado na Passagem de Plantão.
      let diagTxt = (ev && ev.diag) || (l && l.diag) || '';
      if(ev && Array.isArray(ev.diags) && ev.diags.length > 1){
        const extras = ev.diags.slice(1).map(x => x.diag).filter(Boolean);
        if(extras.length) diagTxt = [diagTxt, ...extras].filter(Boolean).join(' + ');
